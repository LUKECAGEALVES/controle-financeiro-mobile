...
(function(root){
  ...
  function simulate(raw){
    ...
    let fgtsA=n(p.fgts_initial_balance),fgtsB=fgtsA,userTotal=n(p.initial_investment),fgtsInjected=0;const rows=[];
    ...
    const scenarioA={total_user_contributed:userTotal,total_fgts_injected:0,fgts_remaining:fgtsA,portfolio_gross:a.gross,ir_withheld:a.tax,portfolio_net:a.net,consolidated_total:totalA,real_consolidated_total:totalA/finalInflation,gross_portfolio_gain:a.gain};
    const scenarioB={total_user_contributed:userTotal,total_fgts_injected:fgtsInjected,fgts_remaining:fgtsB,portfolio_gross:b.gross,ir_withheld:b.tax,portfolio_net:b.net,consolidated_total:totalB,real_consolidated_total:totalB/finalInflation,gross_portfolio_gain:b.gain};
    return {params:p,scenario_a:scenarioA,scenario_b:scenarioB,delta_value:delta,delta_pct:deltaPct,portfolio_real_rate_pct:fisherRealRate(p.portfolio_annual_rate_pct,p.inflation_annual_pct),fgts_real_rate_pct:fisherRealRate(p.fgts_annual_rate_pct,p.inflation_annual_pct),rows};
  }

  function comparisonRows(result){const a=result.scenario_a,b=result.scenario_b;return [['Total aportado pelo usuário',a.total_user_contributed,b.total_user_contributed],['Total injetado do FGTS',a.total_fgts_injected,b.total_fgts_injected],['Saldo FGTS restante',a.fgts_remaining,b.fgts_remaining],['Carteira bruta',a.portfolio_gross,b.portfolio_gross],['IR retido estimado',a.ir_withheld,b.ir_withheld],['Carteira líquida',a.portfolio_net,b.portfolio_net],['Patrimônio total consolidado',a.consolidated_total,b.consolidated_total],['Patrimônio real (poder de compra)',a.real_consolidated_total,b.real_consolidated_total]]}
  function rowsToCsv(rows){const cols=['mes','ano_simulacao','mes_do_ano','aporte_mensal_usuario','decimo_terceiro','plr','aporte_usuario_total_mes','saque_fgts_cenario_b','fgts_a','fgts_b','carteira_bruta_a','carteira_bruta_b','ir_estimado_a','ir_estimado_b','carteira_liquida_a','carteira_liquida_b','patrimonio_total_a','patrimonio_total_b','patrimonio_real_a','patrimonio_real_b'];return cols.join(';')+'\n'+rows.map(r=>cols.map(c=>typeof r[c]==='number'?String(Math.round(r[c]*100)/100).replace('.',','):String(r[c]??'')).join(';')).join('\n')}
