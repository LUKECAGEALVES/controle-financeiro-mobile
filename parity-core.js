...
function periodIncomeQuality(year,month,metrics=null){
  const m=Number(month||0);if(!m||!year||String(year)==='Todos')return {partial:false,provisional:false,warning:'',referenceIncome:0,referenceSource:'',coveragePct:null,reasons:[]};
  const y=Number(year),d=metrics||periodMetrics(y,m);let reference=Math.max(0,Number(data.settings.monthlyIncome||0)),source=reference>0?'Renda mensal configurada':'';
  if(reference<=0){const vals=[];let py=y,pm=m;for(let i=0;i<6;i++){pm--;if(pm===0){pm=12;py--}const prev=periodMetrics(py,pm);if(Number(prev.income||0)>.005)vals.push(Number(prev.income||0))}if(vals.length){reference=vals.reduce((a,b)=>a+b,0)/vals.length;source='Média dos meses anteriores com receita'}}
  const income=Number(d.income||0),expense=Number(d.expense||0),coverage=reference>0?income/reference*100:null,reasons=[];let partial=false,provisional=false;
  const now=new Date(),inProgress=y===now.getFullYear()&&m===now.getMonth()+1&&now.getDate()<new Date(y,m,0).getDate();
  if(inProgress){provisional=true;reasons.push('o mês ainda está em andamento')}
  if(expense>.005&&income<=.005){partial=true;reasons.push('há gastos registrados, mas nenhuma receita registrada no período')}
  else if(reference>0&&expense>.005&&coverage!==null&&coverage<60){partial=true;reasons.push(`a receita registrada representa apenas ${coverage.toFixed(1).replace('.',',')}% da renda de referência`)}
  const warning=partial?`Dados ainda incompletos para percentuais: ${reasons.join('; ')}. Os valores em reais continuam sendo mostrados normalmente.`:provisional?`Período em andamento: ${reasons.join('; ')}. Percentuais calculados sobre o realizado podem mudar até o fechamento do mês.`:'';
  return {partial,provisional,warning,referenceIncome:moneyRound(reference),referenceSource:source,coveragePct:coverage,reasons};
}
...
function financialHealthBase(recent,configuredIncome=0){
  const configured=Math.max(0,Number(configuredIncome||0));
  const incomeMonths=(recent||[]).filter(x=>Number(x.income||0)>.005),expenseMonths=(recent||[]).filter(x=>Number(x.expense||0)>.005),essentialMonths=(recent||[]).filter(x=>Number(x.essential||0)>.005);
  const observedIncome=incomeMonths.length?incomeMonths.reduce((s,x)=>s+Number(x.income||0),0)/incomeMonths.length:0;
  const incomeReference=configured>0?configured:observedIncome;
  const incomeSource=configured>0?'Renda mensal configurada':observedIncome>0?'Média dos meses com renda registrada':'Sem renda de referência';
  const avgExpense=expenseMonths.length?expenseMonths.reduce((s,x)=>s+Number(x.expense||0),0)/expenseMonths.length:0;
  const avgEssential=essentialMonths.length?essentialMonths.reduce((s,x)=>s+Number(x.essential||0),0)/essentialMonths.length:0;
  const savingsRate=incomeReference?(incomeReference-avgExpense)/incomeReference*100:0,essentialPct=incomeReference?avgEssential/incomeReference*100:0;
  return {incomeReference,observedIncome,avgExpense,avgEssential,savingsRate,essentialPct,incomeSource,incomeMonths:incomeMonths.length,expenseMonths:expenseMonths.length,essentialMonths:essentialMonths.length};
}
function financialHealth(){
  ...
  const base=financialHealthBase(recent,Number(data.settings.monthlyIncome||0)),avgIncome=base.incomeReference,avgExpense=base.avgExpense,avgEssential=base.avgEssential,current=periodMetrics(ly,lm),debt=debtSummary(),reserve=emergencyReserve(),nw=networthSummary();
  const savingsRate=base.savingsRate,debtCommit=avgIncome?debt.monthly/avgIncome*100:0,reserveMonths=avgEssential?reserve.current/avgEssential:0,essentialPct=base.essentialPct,alerts=[];
  if(avgIncome<=0)alerts.push(['Atenção','Cadastre a renda mensal atual ou registre uma receita para calcular as taxas de renda.']);else if(savingsRate<0)alerts.push(['Crítico','Seus gastos médios estão acima da renda de referência.']);else if(savingsRate<10)alerts.push(['Atenção','Sua taxa média de poupança está abaixo de 10%.']);
  if(debtCommit>30)alerts.push(['Atenção',`Parcelas de dívidas comprometem ${debtCommit.toFixed(1).replace('.',',')}% da renda de referência.`]);
  ...
  let score=100;if(avgIncome<=0)score-=20;else if(savingsRate<0)score-=35;else if(savingsRate<10)score-=20;else if(savingsRate<20)score-=10;if(debtCommit>40)score-=30;else if(debtCommit>30)score-=20;else if(debtCommit>20)score-=10;if(reserveMonths<1)score-=25;else if(reserveMonths<3)score-=15;else if(reserveMonths<6)score-=5;score=clamp(score,0,100);
  return {score,avg_income:avgIncome,avg_expense:avgExpense,savings_rate:savingsRate,debt_commitment:debtCommit,reserve_months:reserveMonths,essential_pct:essentialPct,networth:nw.networth,alerts,period:`${monthName(lm)}/${ly}`,income_source:base.incomeSource,income_months:base.incomeMonths,expense_months:base.expenseMonths,observed_income:base.observedIncome};
}
